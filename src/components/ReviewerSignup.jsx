...
const ReviewerSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://www.ajga-journal.org/api/reviewer_signup.php", {
        method: "POST",
      ...
      if (result.success) {
        Swal.fire({
          ...
          navigate("/reviewerlogin"); // Redirect after success
        });
      } else {
        Swal.fire({
          icon: "error",
  ...
  return (
    <SignUpContainer>
      <FormWrapper>
        <h2 style={{ color: "rgba(0, 0, 255, 0.5)" }}>Reviewer Sign-Up</h2>
        ...
        <SwitchText>
          Already have a Reviewer account? <a onClick={() => navigate('/reviewerlogin')}>Log In</a>
        </SwitchText>
      </FormWrapper>
    </SignUpContainer>
  );
